...
from retriever import retrieve_context_hybrid
...
def evaluate_single_retrieval(item: dict, top_k: int = 5) -> dict:
    ...
    results = retrieve_context_hybrid(question, top_k=top_k)
...
def run_retrieval_eval(top_k: int = 5) -> dict:
    ...
    results = [evaluate_single_retrieval(item, top_k=top_k) for item in dataset]
...
def print_report(eval_results: dict):
    print("=" * 60)
    print(f"HYBRID RETRIEVAL EVALUATION REPORT (top_k={eval_results['top_k']})")
    print("=" * 60)
